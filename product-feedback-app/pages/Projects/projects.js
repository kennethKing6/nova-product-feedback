...
import { firebaseAuth, firebaseApp } from '../../firebase/firebase'
import firebase from "firebase/app";
import "firebase/auth";
import { getFirebaseToken } from '../../firebase/firebaseUtils'
import { createNewProject, getUsersProjects } from '../../apis/projects/index'
...
export default function Projects() {
  ...
  const getProjects = async () => {
    try {
      ...
      console.log(token)
      const currentUserEmail = firebaseAuth && firebaseAuth.currentUser? firebaseAuth.currentUser.email :''
      const projects = await getUsersProjects(token, currentUserEmail)
  ...
  return (
    <projectsContext.Provider value={context}>
      <div className='container-fluid' style={{ color: '#3A4374', minHeight: '100vh', backgroundColor: '#F2F4FE' }}>
        <div className="d-flex flex-row-reverse">
          <div className="p-2"><button type="button" className="btn btn-primary btn-lg" data-bs-toggle="modal" data-bs-target="#exampleModal">Add New Project</button></div>
          <div className="p-2"><button type="button" className="btn  btn-lg" onClick={() => {
        ...
        <div className='row ' style={{ minHeight: '90vh' }}>{Array.isArray(projectsList) && projectsList.length > 0 ? <ProjectItem /> :
          <div className='d-flex justify-content-center align-items-center'> <div style={{ width: 100 }} className='align-self-center'><EmptyImage /></div> </div>}</div>
        <AddProjectModal />
      </div>
...
function ProjectItem() {
  ...
  return (
    <div className='container w-75' >
      {/**  
       * Projects the admin created 
      **/}
      <div>
        <h3 style={{ paddingLeft: '7%' }}>My Projects</h3>
        <div className='row d-flex justify-content-center'>
          {projectsList.map((project, i) => {
            const { name, contributors, admin } = project

            return (
              <div className="card" style={{ width: '18rem', height: '7rem', margin: '1%', backgroundColor: '#373F68', cursor: 'pointer' }}
                onClick={() => router.push("../projectDetails")}>
                <div className="card-body">
                  <h5 className="card-title" style={{ color: 'whitesmoke', textTransform: 'capitalize' }}>{name}</h5>
                  <p className="card-text" style={{ color: 'whitesmoke', opacity: 0.7 }}>{contributors.length} contributors</p>
                </div>
              </div>
            )
          })}
...
function AddProjectModal() {
  ...
  const { reload, setReload } = useContext(projectsContext)
  ...
  useEffect(()=>{
    try{
      setChipData(firebaseAuth.currentUser.email)
    }catch(err){}
  },[])

  return (
    <projectModalContext.Provider value={context}>
      <div className="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div className="modal-dialog">
          <div className="modal-content">
            <div className="modal-header">
              <h1 className="modal-title fs-5" id="exampleModalLabel">Create A New Project</h1>
              <button type="button" id='close' className="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div className="modal-body">

              <div className="form-floating mb-3">
                <input type="text" className="form-control" id="floatingInput" value={projectName} onChange={(e) => setProjectName(e.target.value)} />
            ...
            <div className="modal-footer">
              <button type="button" className="btn btn-primary" onClick={async () => {
                try {
                  ...
                  await createNewProject(token, currentUserEmail, projectName, chipData)
                  document.getElementById('close').click()
                  setReload(!reload)
                } catch (err) {
                  console.log("GGGGGGGGGGGGG: error", err)
...
function ChipsArray() {
  const { inputData, setInputData, chipData, setChipData } = useContext(projectModalContext)

  const handleDelete = (chipToDelete) => () => {
    setChipData((chips) => chips.filter((element) => element !== chipToDelete));
  };
    ...
    >
      <div className='row w-100 mb-3'>
        <div className="form-floating col-7">
          <input type="text" value={inputData} className="form-control mb-2 w-100" id="floatingInput" onChange={(e) => setInputData(e.target.value)} />
          <label for="floatingInput">contributor</label>
        </div>
      ...
      {Array.isArray(chipData) && chipData.map((data, index) => {
        let icon;
        const currentUserEmail = firebaseAuth && firebaseAuth.currentUser? firebaseAuth.currentUser.email : ''
        ...
        return (
          <ChipItem key={index}>
            <Chip
              icon={icon}
              label={data}
              onDelete={currentUserEmail === data ? null : handleDelete(data)}
